...
class App extends Component {
  ...
  handleSearch = query => {
    ...
    setTimeout(() => {
      axios
        .get(
          `https://api.themoviedb.org/3/search/movie?query=${query}&api_key=eea80f47c877f72cbd8716b40d5c3935`
        )
        .then(response => {
  ...
  searchbyFilter = () => {
    ...
    setTimeout(() => {
      axios
        .get(
          `https://api.themoviedb.org/3/discover/movie?api_key=eea80f47c877f72cbd8716b40d5c3935&with_genres=${params}`
        )
        .then(response => {
          this.setState({ movies: response.data.results });
        });
  ...
  componentDidMount() {
    axios
      .get(
        `https://api.themoviedb.org/3/genre/movie/list?api_key=eea80f47c877f72cbd8716b40d5c3935&language=en-US`
      )
      .then(response => {
        this.setState({
      ...
      <div>
        <div>
          <Navbar
            ...
            counter={this.state.selectedMovies.length}
          />
        </div>
        <Switch>
          <Route
              ...
              return (
                ...
                >
                  <ShoppingCart
                    ...
                    genres={this.state.genres}
                    resetDetailView={this.resetDetailView}
                  />
                </movieContext.Provider>
              );
            }}
